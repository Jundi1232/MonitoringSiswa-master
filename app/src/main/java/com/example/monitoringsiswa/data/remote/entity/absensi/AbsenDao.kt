...
interface AbsenDao {
    ...
    @Query("Select * from absen where tgl=:tgl and nis=:nis")
    fun getAbsenbyTglNis(tgl:String, nis:String): LiveData<List<Absen>>

    @Query("Select * from absen where tgl=:tgl and idkelas=:kls")
    ...
    suspend fun update(nis: String,tgl: Long, ket:String)
